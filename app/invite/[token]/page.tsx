...
function InviteClaimInner() {
  ...
  async function tryAutoClaim(inv?: InviteDetails | null) {
    ...
    try {
      ...
      // Route by claimed role:
      //   employee → /employee/settings (first-run password setup)
      //   owner / admin / board → /app/dashboard
      // The invite's role is the authoritative source here.
      const claimedRole = String(inv?.role || body?.item?.role || "").toLowerCase();
      setPhase("done");
      setTimeout(() => {
        if (claimedRole === "employee") {
          router.replace("/employee/settings?firstRun=1");
        } else {
