...
function planAccent(plan: Plan) {
  // subtle UI difference; keeps it premium on white
...
export default function EarlyAccessPage() {
  ...
  useEffect(() => {
    const fromStorage = normalizePlan(
      typeof window !== "undefined" ? localStorage.getItem("chiefos_selected_plan") : null
    );
  ...
  useEffect(() => {
    track("early_access_view", {
      plan: planFromUrl || normalizePlan(localStorage.getItem("chiefos_selected_plan")),
      path: "/early-access",
    });
  }, [planFromUrl]);

  const header = useMemo(() => {
    // keep this “premium + truthful”
    ...
    if (selectedPlan === "pro") {
      return {
        ...
        sub: "Crew + Control: approvals, audit trail, board roles, and long retention.",
      };
    }
    if (selectedPlan === "free") {
      return {
        title: "Start Free",
        sub: "Field Capture: build the daily habit, then upgrade when you want faster capture and deeper answers.",
      };
    }
    return {
    ...
    try {
      ...
      const r = await fetch("/api/early-access", {
        ...
        body: JSON.stringify({
          ...
          plan: selectedPlan, // ✅ add on server too
          turnstileToken,
        }),
      });
